...
======================================
Savatli buyurtma + lokatsiya + to'lov + qabul qilish + eslatma + admin panel

Foydalanuvchi:
  🛒 Buyurtma → mahsulot → o'lcham → son(+/-) → savat
  → telefon → 📍 joylashuv (faqat Telegram lokatsiya) → to'lov turi → tasdiq

Admin (siz + tayinlaganlaringiz):
  • Yangi zakaz kelganda «✅ Qabul qilish» tugmasi bilan keladi (lokatsiya bilan)
...
def get_order(oid): 
    return db.execute("SELECT id,ts,user_id,name,username,items,total,phone,lat,lon,pay,status,accepted_by "
...
# ── buyurtma: telefon → lokatsiya → to'lov ────────────
...
async def choose_pay(c: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    cart = data.get("cart", []); total = cart_total(cart)
    u = c.from_user; uname = f"@{u.username}" if u.username else "—"

    oid = save_order({"user_id":u.id,"name":u.full_name,"username":uname,
                      "items":cart,"total":total,"phone":data.get("phone"),
                      "lat":data.get("lat"),"lon":data.get("lon"),"pay":pay})

    # adminlarga: matn + lokatsiya + qabul tugmasi
    ...
    for aid in all_admin_ids():
        try:
            ...
            if data.get("lat") is not None:
                await bot.send_location(aid, data["lat"], data["lon"])
...
async def zakaz_detail(m: Message, bot: Bot):
    ...
    oid,ts,uid,name,uname,items,total,phone,lat,lon,pay,status,acc=row
